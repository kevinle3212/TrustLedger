...
> **Why `NEXT_PUBLIC_TRUSTLEDGER_ADDRESS` must be set in Vercel:**
> `artifacts/deployed-addresses.json` is gitignored and never reaches the CI checkout. The Vercel build has no access to it. The env var is the only mechanism for the frontend to know the contract address. `deploy.yml` keeps it up to date automatically after every deploy.
