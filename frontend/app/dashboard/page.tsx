...
function ContractCard({
  ...
  const status = contract.status as number;
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5 flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {isFreelancer && status === 0 && (
          <>
            <ActionButton label="Accept" contractId={id} functionName="acceptContract" />
            <ActionButton label="Reject" contractId={id} functionName="rejectContract" />
          </>
        )}
        {/* Client actions */}
        {isClient && status === 2 && (
          <>
            <ActionButton label="Approve Work" contractId={id} functionName="approveWork" />
            <ActionButton label="Dispute" contractId={id} functionName="disputeWork" />
          </>
        )}
        {isClient && status === 1 && now > contract.projectDeadline && (
          <ActionButton label="Reclaim (Deadline Missed)" contractId={id} functionName="claimAfterDeadlineMiss" />
        )}
        {/* Auto-release after acceptance window */}
        {status === 2 && now > contract.acceptanceDeadline && contract.acceptanceDeadline > BigInt(0) && (
          <ActionButton label="Release After Window" contractId={id} functionName="claimAfterAcceptanceWindow" />
        )}
        {/* Warranty claim */}
        {isFreelancer && status === 3 && contract.holdBackAmount > BigInt(0) && now > contract.warrantyDeadline && (
          <ActionButton label="Claim Warranty Funds" contractId={id} functionName="claimWarrantyFunds" />
        )}
      </div>
    </div>
  );
...
function ContractList({ address }: { address: `0x${string}` }) {
  ...
  if (total === 0) {
    return (
      <div className="text-center py-16 text-gray-500">
        ...
        <Link href="/create" className="mt-3 inline-block text-indigo-400 hover:text-indigo-300 text-sm underline">
...
function SingleContract({ id, address }: { id: bigint; address: `0x${string}` }) {
  ...
  const c = contract as EscrowContract;
  const isParticipant =
