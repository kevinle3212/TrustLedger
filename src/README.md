...
| `npm run debug:files`   | TypeScript trace + CPU profile — outputs `trace/` and `profile.cpuprofile` for compile-time debugging |
...
| Build                | `npm run build` with `NEXT_BASE_PATH=/TrustLedger` |
