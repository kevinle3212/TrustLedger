...
legal documents, CI config, security policy) through **review controls** you
configure in GitHub, plus `CODEOWNERS`.
...
- Require status checks to pass (select the CI checks: lint, type-check, tests,
  build, security).
...
- Legal draft markdown at the repository root must not be edited without
  explicit owner approval (see root `CLAUDE.md`).
